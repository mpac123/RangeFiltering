#ifndef BloomedSplash_BUILDER_BLOOMED_SPLASH_H_
#define BloomedSplash_BUILDER_BLOOMED_SPLASH_H_
...
namespace range_filtering_bloomed_splash {
    class BloomedSplashBuilder {
    public:
        BloomedSplashBuilder() : sparse_start_level_(0) {};
        explicit BloomedSplashBuilder(bool include_dense, uint32_t sparse_dense_ratio)
                : include_dense_(include_dense), sparse_dense_ratio_(sparse_dense_ratio),
                  sparse_start_level_(0)  {};

        ~BloomedSplashBuilder() {};
    ...
    void BloomedSplashBuilder::build(const std::vector<std::string>& keys) {
        assert(keys.size() > 0);
        buildSparse(keys);
        if (include_dense_) {
    ...
    void BloomedSplashBuilder::buildSparse(const std::vector<std::string>& keys) {
    ...
    level_t BloomedSplashBuilder::skipCommonPrefix(const std::string& key) {
        level_t level = 0;
    ...
    level_t BloomedSplashBuilder::insertKeyBytesToTrie(const std::string& key, const std::string& next_key, const level_t start_level) {
    ...
    inline bool BloomedSplashBuilder::isCharCommonPrefix(const label_t c, const level_t level) const {
        return (level < getTreeHeight())
               && (!is_last_item_terminator_[level])
               && (c == labels_[level].back());
    }

    inline bool BloomedSplashBuilder::isLevelEmpty(const level_t level) const {
        return (level >= getTreeHeight()) || (labels_[level].size() == 0);
    }

    inline void BloomedSplashBuilder::moveToNextItemSlot(const level_t level) {
    ...
    void BloomedSplashBuilder::insertKeyByte(const char c, const level_t level, const bool is_start_of_node, const bool is_term) {
    ...
    inline void BloomedSplashBuilder::storeSuffix(const level_t level, const word_t suffix) {
    ...
    inline void BloomedSplashBuilder::determineCutoffLevel() {
    ...
    inline uint64_t BloomedSplashBuilder::computeDenseMem(const level_t downto_level) const {
    ...
    inline uint64_t BloomedSplashBuilder::computeSparseMem(const level_t start_level) const {
        uint64_t mem = 0;
    ...
    void BloomedSplashBuilder::buildDense() {
    ...
    void BloomedSplashBuilder::initDenseVectors(const level_t level) {
    ...
    void BloomedSplashBuilder::setLabelAndChildIndicatorBitmap(const level_t level,
    ...
    void BloomedSplashBuilder::addLevel() {
    ...
    position_t BloomedSplashBuilder::getNumItems(const level_t level) const {
        return labels_[level].size();
    }

    bool BloomedSplashBuilder::isStartOfNode(const level_t level, const position_t pos) const {
        return readBit(louds_bits_[level], pos);
    }

    bool BloomedSplashBuilder::isTerminator(const level_t level, const position_t pos) const {
        label_t label = labels_[level][pos];
        return ((label == kTerminator) && !readBit(child_indicator_bits_[level], pos));
    }
};

#endif // BloomedSplash_BUILDER_H_