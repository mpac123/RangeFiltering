...
namespace range_filtering_bloomed_splash {
    ...
    namespace buildertest {
        ...
        class BloomedSplashBuilderUnitTest : public ::testing::Test {
        public:
            virtual void SetUp () {
        ...
        void BloomedSplashBuilderUnitTest::truncateSuffixes(const std::vector<std::string> &keys, std::vector<std::string> &keys_trunc) {
            assert(keys.size() > 1);
        ...
        void BloomedSplashBuilderUnitTest::fillinInts() {
...
//debug
        ...
        void BloomedSplashBuilderUnitTest::printDenseNode(level_t level, position_t node_num) {
            printIndent(level);
            ...
            for (position_t i = 0; i < kFanout; i++) {
                if (BloomedSplashBuilder::readBit(builder_->getBitmapLabels()[level], node_num * kFanout + i)) {
            ...
            for (position_t i = 0; i < kFanout; i++) {
                if (BloomedSplashBuilder::readBit(builder_->getBitmapLabels()[level], node_num * kFanout + i)) {
                    if (BloomedSplashBuilder::readBit(builder_->getBitmapChildIndicatorBits()[level], node_num * kFanout + i))
                        std::cout << "1 ";
                    else
                        std::cout << "0 ";
                }
            ...
            printIndent(level);
            if (BloomedSplashBuilder::readBit(builder_->getPrefixkeyIndicatorBits()[level], node_num))
                std::cout << "1 ";
            else
                std::cout << "0 ";
            std::cout << "\n";
        ...
        void BloomedSplashBuilderUnitTest::printSparseNode(level_t level, position_t pos) {
            printIndent(level);
            ...
            while (!is_end_of_node && pos < builder_->getLabels()[level].size()) {
                ...
                pos++;
                is_end_of_node = BloomedSplashBuilder::readBit(builder_->getLoudsBits()[level], pos);
            }
            std::cout << "\n";
            ...
            pos = start_pos;
            while (!is_end_of_node && pos < builder_->getLabels()[level].size()) {
                bool has_child = BloomedSplashBuilder::readBit(builder_->getChildIndicatorBits()[level], pos);
                if (has_child)
                    std::cout << "1 ";
                else
                    std::cout << "0 ";
            ...
            pos = start_pos;
            while (!is_end_of_node && pos < builder_->getLabels()[level].size()) {
                bool louds_bit = BloomedSplashBuilder::readBit(builder_->getLoudsBits()[level], pos);
                if (louds_bit)
                    std::cout << "1 ";
                else
                    std::cout << "0 ";
        ...
        bool BloomedSplashBuilderUnitTest::DoesPrefixMatchInTrunc(const std::vector<std::string> &keys_trunc, int i, int j, int len) {
        ...
        void BloomedSplashBuilderUnitTest::testSparse(const std::vector<std::string> &keys,
                                             const std::vector<std::string> &keys_trunc) {
...
//                    ASSERT_TRUE(exist_in_node);
        ...
        void BloomedSplashBuilderUnitTest::testDense() {
            for (level_t level = 0; level < builder_->getSparseStartLevel(); level++) {
                ...
                for (unsigned i = 0; i < builder_->getLabels()[level].size(); i++) {
                    bool is_node_start = BloomedSplashBuilder::readBit(builder_->getLoudsBits()[level], i);
                    if (is_node_start)
                        node_num++;

                    label_t label = builder_->getLabels()[level][i];
                    bool exist_in_node = BloomedSplashBuilder::readBit(builder_->getBitmapLabels()[level], node_num * kFanout + label);
                    bool has_child_sparse = BloomedSplashBuilder::readBit(builder_->getChildIndicatorBits()[level], i);
                    bool has_child_dense = BloomedSplashBuilder::readBit(builder_->getBitmapChildIndicatorBits()[level], node_num * kFanout + label);

                    // prefixkey indicator test
                    if (is_node_start) {
                        bool prefixkey_indicator = BloomedSplashBuilder::readBit(builder_->getPrefixkeyIndicatorBits()[level], node_num);
                    ...
                    if (is_node_start) {
                        if (node_num > 0) {
                            for (unsigned c = prev_label + 1; c < kFanout; c++) {
                                exist_in_node = BloomedSplashBuilder::readBit(builder_->getBitmapLabels()[level], (node_num - 1) * kFanout + c);
                                ASSERT_FALSE(exist_in_node);
                                has_child_dense = BloomedSplashBuilder::readBit(builder_->getBitmapChildIndicatorBits()[level], (node_num - 1) * kFanout + c);
                                ASSERT_FALSE(has_child_dense);
                            }
                        }
                        for (unsigned c = 0; c < (unsigned)label; c++) {
                            exist_in_node = BloomedSplashBuilder::readBit(builder_->getBitmapLabels()[level], node_num * kFanout + c);
                            ASSERT_FALSE(exist_in_node);
                            has_child_dense = BloomedSplashBuilder::readBit(builder_->getBitmapChildIndicatorBits()[level], node_num * kFanout + c);
                            ASSERT_FALSE(has_child_dense);
                        }
                    } else {
...
//        TEST_F (BloomedSplashBuilderUnitTest, buildSparseStringTest) {
...
//        }
//
//        TEST_F (BloomedSplashBuilderUnitTest, buildSparseDuplicateTest) {
//            bool include_dense = false;
//            uint32_t sparse_dense_ratio = 0;
//            builder_ = new BloomedSplashBuilder(include_dense, sparse_dense_ratio);
...
//        }
        ...
        TEST_F (BloomedSplashBuilderUnitTest, buildSparseIntTest) {
            bool include_dense = false;
            uint32_t sparse_dense_ratio = 0;
            builder_ = new BloomedSplashBuilder(include_dense, sparse_dense_ratio);
        ...
        TEST_F (BloomedSplashBuilderUnitTest, buildDenseStringTest) {
        ...
        TEST_F (BloomedSplashBuilderUnitTest, buildDenseIntTest) {
            bool include_dense = true;
            uint32_t sparse_dense_ratio = 0;
            builder_ = new BloomedSplashBuilder(include_dense, sparse_dense_ratio);
            builder_->build(ints_);
...
int main (int argc, char** argv) {
    ...
    range_filtering_bloomed_splash::buildertest::loadWordList();
