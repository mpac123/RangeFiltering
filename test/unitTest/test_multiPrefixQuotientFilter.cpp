...
namespace range_filtering {
    namespace multi_prefix_quotient_filter_test {

    class MultiPrefixQuotientFilterUnitTest : public ::testing::Test {

    };

        TEST_F(MultiPrefixQuotientFilterUnitTest, emptyTrie) {
            auto keys = std::vector<std::string>();
            auto trie = MultiPrefixQuotientFilter(keys, 5);

            ASSERT_FALSE(trie.lookupPrefix("anything"));
            ASSERT_TRUE(trie.lookupPrefix(""));
        }

        TEST_F(MultiPrefixQuotientFilterUnitTest, simpleTrie) {
            ...
            auto trie = MultiPrefixQuotientFilter(keys, 6);
        ...
        TEST_F(MultiPrefixQuotientFilterUnitTest, compactTrie) {
            std::vector<std::string> keys = {
                    "ba",
                    "baba",
                    "babca",
