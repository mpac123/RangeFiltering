...
def run_SuRFDense_bench(dist, qt, workload_dir, SR_suffix_size):
    cmd = ["../build/bench/range_filtering_bench", str(dist), str(qt), str(workload_dir), "surfdense", str(SR_suffix_size)]
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    return pd.read_csv(p.stdout, delimiter="\t")
...
results_dir = "range-filtering-results/100k_26_15_3__surf_louds_times"
...
for dist in distributions:
    for qt in querytypes:
        ...
        df_surfd_0 = run_SuRFDense_bench(dist, qt, workload_dir, 0)
        #df_surfd_4 = run_SuRFDense_bench(dist, qt, workload_dir, 4)
        #df_surfd_8 = run_SuRFDense_bench(dist, qt, workload_dir, 8)
        ...
        #df_surfd_0.rename(columns={'FPR': 'SuRF Base'}, inplace=True)
        #df_surfd_4.rename(columns={'FPR': 'SuRF Real s=4'}, inplace=True)
        #df_surfd_8.rename(columns={'FPR': 'SuRF Real s=8'}, inplace=True)
        ...
        #     df_surfd_0[['Memory usage', 'SuRF Base']],
        #     df_surfd_4[['Memory usage', 'SuRF Real s=4']],
        #     df_surfd_8[['Memory usage', 'SuRF Real s=8']],
        #     df_fst[['Memory usage', 'FST']]]
        df = df_surfd_0[['Suffix size', 'Query time', 'Memory usage']]
