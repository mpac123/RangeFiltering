...
#include "QuotientTrie.hpp"
#include <fstream>

namespace range_filters {
    namespace quotient_trie_test {

        static const std::string kFilePath = "/home/mapac/Coding/RangeFiltering/succinct_trie/test/words.txt";
        static std::vector<std::string> words;
        static const int kWordTestSize = 234369;

        class QuotientTrieUnitTest : public ::testing::Test {
        };

        TEST_F(QuotientTrieUnitTest, simpleKeysSet) {
            ...
            auto trie = QuotientTrie(keys, 1, 0.2);
        ...
        TEST_F (QuotientTrieUnitTest, lookupRangeWordTest) {
            auto filter = new QuotientTrie(words, 1, 0.2);
...
int main (int argc, char** argv) {
    ...
    range_filters::quotient_trie_test::loadWordList();
