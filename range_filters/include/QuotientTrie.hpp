...
class QuotientTrie : public range_filtering::RangeFilter {
    public:
        ...
        QuotientTrie(std::vector<std::string> &keys, uint32_t topLaterMaxLength, float filledInFraction);
    ...
    private:
        ...
        void countKeysAndFillInCharUsageMask(range_filtering::RestrainedSplashyTrie& trie, uint64_t& prefix_cnt);
        ...
        void populateFilter(range_filtering::RestrainedSplashyTrie::TrieNode* node, std::string& prefix);
        ...
        std::string findPrefixInTopLayer(const std::string& key);
    ...
    range_filters::QuotientTrie::QuotientTrie(std::vector<std::string> &keys, uint32_t topLayerMaxLength,
        ...
        if (restraintType == RestraintType::relative) {
            restrained_trie = range_filtering::RestrainedSplashyTrie(keys, 0, cutOffCoefficient,
                              range_filtering::RestraintType::relative, 0, relativeRestraintType);
        } else {
            restrained_trie = range_filtering::RestrainedSplashyTrie(keys, 0, cutOffCoefficient,
                              range_filtering::RestraintType::absolute, absoluteRestraintType, 0);
        }
        uint64_t prefix_cnt = 0;
        countKeysAndFillInCharUsageMask(restrained_trie, prefix_cnt);
        zero_after_one_mask_ = calculateOneToZeroMask(char_usage_mask_);
        ...
        // Populate table with prefixes
        std::string prefix = "";
        populateFilter(restrained_trie.root, prefix);
    }

    void QuotientTrie::countKeysAndFillInCharUsageMask(range_filtering::RestrainedSplashyTrie &trie, uint64_t& prefix_cnt) {
        std::string new_string = "";
        traverseNode(trie.root, new_string, prefix_cnt);
    }

    void QuotientTrie::countKeysAndFillInCharUsageMask(range_filtering::Trie &trie, uint64_t& prefix_cnt) {
        std::string new_string = "";
        traverseNode(trie.root, new_string, prefix_cnt);
    }
    ...
    void QuotientTrie::traverseNode(range_filtering::Trie::TrieNode* node,
                                    std::string& prefix, uint64_t& prefix_cnt) {
        if (prefix.length() == top_layer_max_length_) {
            prefix_list_.push_back(prefix);
        }
    ...
    void QuotientTrie::populateFilter(range_filtering::Trie::TrieNode* node, std::string& prefix) {
        if (prefix.length() < top_layer_max_length_) {
            for (auto child : node->children) {
                prefix += child.first;
                populateFilter(child.second, prefix);
    ...
    void QuotientTrie::insertElement(uint64_t idx, char letter) {
    ...
    uint64_t QuotientTrie::getElement(uint64_t idx) {
    ...
    void QuotientTrie::setElement(uint64_t idx, uint64_t elt) {
        size_t bitpos = elem_bits_ * idx;
        size_t tabpos = bitpos / 64;
        size_t slotpos = bitpos % 64;
        int spillbits = (slotpos + elem_bits_) - 64;
    ...
    uint64_t QuotientTrie::findRunIndex(uint64_t idx) {
    ...
    void QuotientTrie::insertAndShift(uint64_t s, uint64_t elt) {
        uint64_t prev;
        uint64_t curr = elt;
        bool empty;
    ...
    bool QuotientTrie::lookupRange(const std::string &leftKey, const std::string &rightKey) {
    ...
    bool QuotientTrie::lookupRangeRecursively(const std::string &leftKey, const std::string &rightKey) {
        std::string common_prefix;
        for (size_t i = 0; i < std::min(leftKey.length(), rightKey.length()); i++) {
            if (leftKey[i] != rightKey[i]) break;
            common_prefix += leftKey[i];
    ...
    std::pair<bool, std::string> QuotientTrie::queryTopLayer(const std::string &leftKey, const std::string &rightKey) {
    ...
    std::string QuotientTrie::findPrefixInTopLayer(const std::string &key) {
        int a = 0, b = prefix_list_.size() - 1;
        while (a <= b) {
            int k = (a + b) / 2;
            if (prefix_list_[k] == key.substr(0, prefix_list_[k].length())) {
                return prefix_list_[k];
            }
    ...
    int64_t QuotientTrie::findPosPrefixInTopLayer(const std::string &key) {
        int a = 0, b = prefix_list_.size() - 1;
        while (a <= b) {
            int k = (a + b) / 2;
    ...
    bool QuotientTrie::lookupLetter(uint64_t idx, char letter) {
    ...
    bool QuotientTrie::lookupRangeBetweenLetters(uint64_t idx, char letterLeft, char letterRight) {
    ...
    uint64_t QuotientTrie::getMemoryUsage() const {
        uint64_t total_bits = 2 * 256 // char usage mask + zero after one mask
                + 64 + 8 + 64 + 32 + 64 + 32 // masks, counters etc
