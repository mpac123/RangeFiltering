#ifndef RANGE_FILTERING_BLOOMED_RANGE_SPLASH_BUILDER_HPP
#define RANGE_FILTERING_BLOOMED_RANGE_SPLASH_BUILDER_HPP
...
namespace range_filtering_bloomed_range_splash {
    class BloomedSplashRangeBuilder {
    public:
        BloomedSplashRangeBuilder() : sparse_start_level_(0) {};
        explicit BloomedSplashRangeBuilder(bool include_dense, uint32_t sparse_dense_ratio)
                : include_dense_(include_dense), sparse_dense_ratio_(sparse_dense_ratio),
                  sparse_start_level_(0)  {};

        ~BloomedSplashRangeBuilder() {};
    ...
    private:
        ...
        level_t insertKeyBytesToTrie(const std::string& key, const std::string& next_key, const level_t start_level);
    ...
    void BloomedSplashRangeBuilder::build(const std::vector<std::string>& keys) {
        assert(keys.size() > 0);
        buildSparse(keys);
        if (include_dense_) {
    ...
    void BloomedSplashRangeBuilder::buildSparse(const std::vector<std::string>& keys) {
        for (position_t i = 0; i < keys.size(); i++) {
            ...
            while ((i + 1 < keys.size()) && isSameKey(keys[curpos], keys[i+1]))
                i++;
            if (i < keys.size() - 1)
                insertKeyBytesToTrie(keys[curpos], keys[i+1], level);
            else // for last key, there is no successor key in the list
                insertKeyBytesToTrie(keys[curpos], std::string(), level);
        }
    }

    level_t BloomedSplashRangeBuilder::skipCommonPrefix(const std::string& key) {
        level_t level = 0;
    ...
    level_t BloomedSplashRangeBuilder::insertKeyBytesToTrie(const std::string& key, const std::string& next_key, const level_t start_level) {
    ...
    inline bool BloomedSplashRangeBuilder::isCharCommonPrefix(const label_t c, const level_t level) const {
        return (level < getTreeHeight())
               && (!is_last_item_terminator_[level])
               && (c == labels_[level].back());
    }

    inline bool BloomedSplashRangeBuilder::isLevelEmpty(const level_t level) const {
        return (level >= getTreeHeight()) || (labels_[level].size() == 0);
    }

    inline void BloomedSplashRangeBuilder::moveToNextItemSlot(const level_t level) {
    ...
    void BloomedSplashRangeBuilder::insertKeyByte(const char c, const level_t level, const bool is_start_of_node, const bool is_term) {
    ...
    inline void BloomedSplashRangeBuilder::storeSuffix(const level_t level, const word_t suffix) {
    ...
    inline void BloomedSplashRangeBuilder::determineCutoffLevel() {
    ...
    inline uint64_t BloomedSplashRangeBuilder::computeDenseMem(const level_t downto_level) const {
    ...
    inline uint64_t BloomedSplashRangeBuilder::computeSparseMem(const level_t start_level) const {
        uint64_t mem = 0;
    ...
    void BloomedSplashRangeBuilder::buildDense() {
    ...
    void BloomedSplashRangeBuilder::initDenseVectors(const level_t level) {
    ...
    void BloomedSplashRangeBuilder::setLabelAndChildIndicatorBitmap(const level_t level,
    ...
    void BloomedSplashRangeBuilder::addLevel() {
    ...
    position_t BloomedSplashRangeBuilder::getNumItems(const level_t level) const {
        return labels_[level].size();
    }

    bool BloomedSplashRangeBuilder::isStartOfNode(const level_t level, const position_t pos) const {
        return readBit(louds_bits_[level], pos);
    }

    bool BloomedSplashRangeBuilder::isTerminator(const level_t level, const position_t pos) const {
        label_t label = labels_[level][pos];
        return ((label == kTerminator) && !readBit(child_indicator_bits_[level], pos));
    }
}

#endif //RANGE_FILTERING_BLOOMED_RANGE_SPLASH_BUILDER_HPP
