...
namespace range_filtering_splash {
    ...
    namespace fsttest {
        ...
        class SplashUnitTest : public ::testing::Test {
        public:
            virtual void SetUp () {
        ...
        void SplashUnitTest::newSplashWords() {
            fst_ = new Splash(words, SplashRestraintType::none, 0, 0.0, 1.0);
        }

        void SplashUnitTest::newSplashInts() {
                fst_ = new Splash(ints_, SplashRestraintType::none, 0, 0.0, 1.0);
        }

        void SplashUnitTest::truncateWordSuffixes() {
            assert(words.size() > 1);
        ...
        void SplashUnitTest::fillinInts() {
            for (uint64_t i = 0; i < kIntTestBound; i += kIntTestSkip) {
                ints_.push_back(uint64ToString(i));
            }
        }

        void SplashUnitTest::testSerialize() {
            data_ = fst_->serialize();
            fst_->destroy();
            delete fst_;
            char* data = data_;
            fst_ = Splash::deSerialize(data);
        }

        void SplashUnitTest::testLookupWord() {
            for (unsigned i = 0; i < words.size(); i++) {
        ...
        TEST_F (SplashUnitTest, IntStringConvertTest) {
            for (uint64_t i = 0; i < kIntTestBound; i++) {
                ASSERT_EQ(i, stringToUint64(uint64ToString(i)));
            }
        }

        TEST_F (SplashUnitTest, lookupWordTest) {
            newSplashWords();
            testLookupWord();
            fst_->destroy();
            delete fst_;
        ...
        TEST_F (SplashUnitTest, serializeTest) {
            newSplashWords();
            testSerialize();
            testLookupWord();
        }

        TEST_F (SplashUnitTest, lookupIntTest) {
            newSplashInts();
            for (uint64_t i = 0; i < kIntTestBound; i += kIntTestSkip) {
        ...
        TEST_F (SplashUnitTest, moveToKeyGreaterThanWordTest) {
            newSplashWords();
            bool inclusive = true;
            for (int i = 0; i < 2; i++) {
                if (i == 1)
        ...
        TEST_F (SplashUnitTest, moveToKeyGreaterThanIntTest) {
            newSplashInts();
            bool inclusive = true;
            for (int i = 0; i < 2; i++) {
                if (i == 1)
                ...
                for (uint64_t j = 0; j < kIntTestBound - 1; j++) {
                    Splash::Iter iter = fst_->moveToKeyGreaterThan(uint64ToString(j), inclusive);
                ...
                Splash::Iter iter = fst_->moveToKeyGreaterThan(uint64ToString(kIntTestBound - 1), inclusive);
                if (iter.getFpFlag()) {
                    ASSERT_TRUE(iter.isValid());
                    unsigned bitlen;
                    std::string iter_key = iter.getKeyWithSuffix(&bitlen);
        ...
        TEST_F (SplashUnitTest, moveToKeyLessThanWordTest) {
            newSplashWords();
            bool inclusive = true;
            for (int i = 0; i < 2; i++) {
                if (i == 1)
                ...
                for (unsigned j = 1; j < words.size(); j++) {
                    Splash::Iter iter = fst_->moveToKeyLessThan(words[j], inclusive);
                ...
                Splash::Iter iter = fst_->moveToKeyLessThan(words[0], inclusive);
                if (iter.getFpFlag()) {
                    ASSERT_TRUE(iter.isValid());
                    unsigned bitlen;
                    std::string iter_key = iter.getKeyWithSuffix(&bitlen);
        ...
        TEST_F (SplashUnitTest, moveToKeyLessThanIntTest) {
            newSplashInts();
            bool inclusive = true;
            for (int i = 0; i < 2; i++) {
                if (i == 1)
                ...
                for (uint64_t j = kIntTestSkip; j < kIntTestBound; j++) {
                    Splash::Iter iter = fst_->moveToKeyLessThan(uint64ToString(j), inclusive);
                ...
                Splash::Iter iter = fst_->moveToKeyLessThan(uint64ToString(0), inclusive);
                if (iter.getFpFlag()) {
                    ASSERT_TRUE(iter.isValid());
                    unsigned bitlen;
                    std::string iter_key = iter.getKeyWithSuffix(&bitlen);
        ...
        TEST_F (SplashUnitTest, IteratorIncrementWordTest) {
            newSplashWords();
            bool inclusive = true;
            Splash::Iter iter = fst_->moveToKeyGreaterThan(words[0], inclusive);
        ...
        TEST_F (SplashUnitTest, IteratorIncrementIntTest) {
            newSplashInts();
            bool inclusive = true;
            Splash::Iter iter = fst_->moveToKeyGreaterThan(uint64ToString(0), inclusive);
        ...
        TEST_F (SplashUnitTest, IteratorDecrementWordTest) {
            newSplashWords();
            bool inclusive = true;
            Splash::Iter iter = fst_->moveToKeyGreaterThan(words[words.size() - 1], inclusive);
        ...
        TEST_F (SplashUnitTest, IteratorDecrementIntTest) {
            newSplashInts();
            bool inclusive = true;
            Splash::Iter iter = fst_->moveToKeyGreaterThan(uint64ToString(kIntTestBound - kIntTestSkip), inclusive);
        ...
        TEST_F (SplashUnitTest, lookupRangeWordTest) {
            newSplashWords();
        ...
        TEST_F (SplashUnitTest, lookupRangeIntTest) {
            newSplashInts();
            for (uint64_t i = 0; i < kIntTestBound; i++) {
        ...
        TEST_F (SplashUnitTest, approxCountWordTest) {
            newSplashWords();
            ...
            for (int i = 0; i < num_start_indexes; i++) {
                ...
                for (int j = s; j < kWordTestSize; j++) {
                    Splash::Iter iter = fst_->moveToKeyGreaterThan(words[s], true);
                    Splash::Iter iter2 = fst_->moveToKeyGreaterThan(words[j], true);
        ...
        TEST_F (SplashUnitTest, approxCountIntTest) {
            fst_ = new Splash(ints_, kIncludeDense, 256, SplashRestraintType::none, 0, 0., 1.);
            const int num_start_indexes = 5;
            const int start_indexes[num_start_indexes] =
            ...
            for (int i = 0; i < num_start_indexes; i++) {
                ...
                for (int j = s; j < kIntTestBound; j += kIntTestSkip) {
                    Splash::Iter iter = fst_->moveToKeyGreaterThan(uint64ToString(s), true);
                    Splash::Iter iter2 = fst_->moveToKeyGreaterThan(uint64ToString(j), true);
                    uint64_t count = fst_->approxCount(&iter, &iter2);
        ...
        TEST_F(SplashUnitTest, words) {
            auto trie = Splash(words, SplashRestraintType::none, 0, 0., 1.);
        ...
        TEST_F(SplashUnitTest, bigTrie) {
            ...
            auto trie = Splash(keys, SplashRestraintType::none, 0, 0., 1.);
        ...
        TEST_F(SplashUnitTest, absolutelyRestrained) {
            std::vector<std::string> keys = {
                    "f",
                    "far",
                    "fast",
            ...
            auto trie = Splash(keys, SplashRestraintType::absolute, 2, 0.0, 1.0);
            ...
            ASSERT_TRUE(trie.lookupPrefix("sies"));
            ...
            ASSERT_TRUE(trie.lookupPrefix("siesra"));
        }

        TEST_F(SplashUnitTest, relativelyRestrained) {
            std::vector<std::string> keys = {
                    "f",
                    "far",
                    "fast",
            ...
            auto trie = Splash(keys, SplashRestraintType::relative, 0, 0.2, 1.0);
            ...
            ASSERT_FALSE(trie.lookupPrefix("sier"));
            ASSERT_TRUE(trie.lookupPrefix("siesta"));
            ASSERT_TRUE(trie.lookupPrefix("sieste"));
            ASSERT_TRUE(trie.lookupPrefix("siestt"));

        }

        TEST_F(SplashUnitTest, relativelyRestrainedAndNoCutOffCoefficient) {
            std::vector<std::string> keys = {
                    "f",
                    "far",
                    "fast",
            ...
            // due to siesta and siestas and cut-off-coeff=1.0:
            ASSERT_FALSE(trie.lookupPrefix("siesr")); // not false positive this time
            ASSERT_FALSE(trie.lookupPrefix("sieste"));
            ASSERT_FALSE(trie.lookupPrefix("siestan")); // not a false positive!!!
            ASSERT_TRUE(trie.lookupPrefix("siesta"));
            ASSERT_TRUE(trie.lookupPrefix("siestas"));
        }

        TEST_F(SplashUnitTest, relativelyRestrainedAndCutOffCoefficient) {
            std::vector<std::string> keys = {
                    "f",
                    "far",
                    "fast",
