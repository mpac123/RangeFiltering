#ifndef RANGE_FILTERING_SPLASH_LOUDS_SPARSE_HPP
#define RANGE_FILTERING_SPLASH_LOUDS_SPARSE_HPP
...
namespace range_filtering_splash {
    ...
    public:
        LoudsSparse() {};
        LoudsSparse(const SplashBuilder* builder);
    ...
    LoudsSparse::LoudsSparse(const SplashBuilder* builder) {
        is_exact_ = builder->isExact();
    ...
    bool LoudsSparse::lookupPrefix(const std::string& key, const position_t in_node_num) const {
        ...
        for (level = start_level_; level < key.length(); level++) {
            ...
            // if trie branch terminates
            if (!child_indicator_bits_->readBit(pos))
                return !is_exact_ || level == key.length() - 1;
